...
#### 如何提高统计的速度
- 使用索引查询：应避免造成全表查询的（索引失效的情况）：避免null值查询。索引列的数据不要大量重复。
- 数据量的优化：尽量使用数字型字段（字符增加存储开销）、尽可能的使用vachar/nvarchar代替char/nchar(节省存储空间)，避免临时表过多的创建和删除，尽量使用用表变量来代替临时表、尽量避免大数据量的返回。
- 分区表的创建 
